...
# 	Fixed some bugs that I forgot to write down
#	Reporting errors will automatically include the current show/commercial being played.
...
def play_video(source, commercials, max_commercials_per_break, start_pos):
	...
	try:
		...
		while (1):
			...
			try:
					...
					if float(position)>=float(commercials[0]) and position>5:
						...
						while(comm_i>=0):
							try:
								...
								report_error("COMM_PLAY_LOOP", [str(err_pos) + " - " + str(exce)])
				...
				#if there was an error playing commercials, report it and resume playing main video
				error_count = error_count + 1
				report_error("COMM_PLAY", [str(err_pos) + " - " + str(ecce)])
				player.show_video()
				player.play()
		...
		if(err_pos!=7.0):
			error_count = error_count + 1
			report_error("PLAY_LOOP", [str(err_pos) + " - " + str(e), "SOURCE", str(source)])
	...
	try:
		...
		report_error("COMM_quit", [str(err_pos) + " - " + str(ex)])
	try:
		player.quit()
	except Exception as exx:
		report_error("PLAY_quit", [str(err_pos) + " - " + str(exx)])
...
def generate_commercials_list(max_time_to_fill):
	...
	#
	# where "length" is a whole number representing the actual length of the video
...
def get_current_channel():
	# looks for a plain text file containing a 'channel' name that can be set in the settings file
	global settings
...
while(1):
	try:
		...
		if folder != None: # folders have been set, load and play video
			...
			if video:
				...
				try:
					...
					if (now.hour >= 2 and now.hour <= 5 and get_uptime() > 36000) or error_count >= 20:
						err_count = 12.1
		...
		report_error("MAIN_LOOP", [valerr.encode('utf-8'), source.encode('utf-8'), folder.encode('utf-8'), err_count.encode('utf-8'), "-".join(err_extra)])
